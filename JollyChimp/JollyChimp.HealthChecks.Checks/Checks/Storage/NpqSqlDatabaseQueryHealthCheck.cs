...
internal sealed class NpqSqlDatabaseQueryHealthCheck : IJollyCheck
{
    
    public HealthCheckDefinition Definition => new()
    {
        Name = "NPQ SQL Database Query",
        Catergory = HealthCheckCategoryConstants.Storage,
        IsEnabled = true,
        Type = HealthCheckType.NpqSqlDatabaseQuery,
        Parameters = new()
        {
    ...
    public IHealthChecksBuilder AddHealthCheck(IHealthChecksBuilder builder, HealthCheckStartupConfig config)
    {
        ...
        var query = config.Parameters.ExtractRequiredParameter("QUERY");
